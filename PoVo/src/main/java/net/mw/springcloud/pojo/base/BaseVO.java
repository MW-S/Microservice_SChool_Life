package net.mw.springcloud.pojo.base;

import lombok.Data;
import net.mw.springcloud.utils.NullHelper;
...
public class BaseVO {
    ...
    public <T> T voToPo(final Class<T> poClass) {
        ...
        try {
            ...
            if (tempList.size() > 1) {
                poObj = poClass.getDeclaredConstructor().newInstance();
                ...
                for (final Object key : voMap.keySet()) {
                    ...
                    if (value != null && !value.getClass().isAssignableFrom(Class.class) && !"".equals(value)) {
                        Method method= poMap.getWriteMethod(keyName);
                        method.invoke(poObj,castVoParam(value, poMap.getType(keyName)));
                    }
                }
            }
    ...
    public <T> void poToVo(final T po) {
        try {
            ...
            for (final Object key : poMap.keySet()) {
                value = poMap.get(key);
                keyName = key.toString();
                if (value != null&& !value.getClass().isAssignableFrom(Class.class)) {
                    Method method= voMap.getWriteMethod(keyName);
                    method.invoke(this,castPoParam(value));
                }
            }
        } catch (final Exception e) {
            e.printStackTrace();
