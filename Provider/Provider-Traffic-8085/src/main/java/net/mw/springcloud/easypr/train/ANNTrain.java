package net.mw.springcloud.easypr.train;
...
import net.mw.springcloud.easypr.core.CoreFunc.Direction;
import net.mw.springcloud.easypr.util.Convert;
import net.mw.springcloud.easypr.util.Util;
...
public class ANNTrain {
    ...
    public Mat features(Mat in, int sizeData) {
        ...
        Mat out = new Mat(1, numCols, CV_32F);
        // Asign values to feature,ANN的样本特征为水平、垂直直方图和低分辨率图像所组成的矢量
        int j = 0;
        for (int i = 0; i < vhist.length; i++, ++j) {
        	BytePointer bytePointer = out.ptr(j);
        	bytePointer.put(Convert.getBytes(vhist[i]));
//        	 out.put(0, j, Convert.getBytes(vhist[i]));
        }
        for (int i = 0; i < hhist.length; i++, ++j) {
        	BytePointer bytePointer = out.ptr(j);
        	bytePointer.put(Convert.getBytes(hhist[i]));
        }
        for (int x = 0; x < lowData.cols(); x++) {
            for (int y = 0; y < lowData.rows(); y++, ++j) {
            	BytePointer bytePointer, valBytePointer = lowData.ptr(x, y);
                float val = valBytePointer.get() & 0xFF;
                bytePointer =out.ptr(j);
                bytePointer.put(Convert.getBytes(val));
            }
        }
        // if(DEBUG)
        // cout << out << "\n===========================================\n";
    ...
    public void annTrain(Mat TrainData, Mat classes, int nNeruns) {
        ...
        BytePointer bytePointer0 = layers.ptr(0)
        , bytePointer1 = layers.ptr(1)
        , bytePointer2 = layers.ptr(2);
        bytePointer0.put(Convert.getBytes(TrainData.cols()));
        bytePointer1.put(Convert.getBytes(nNeruns));
        bytePointer2.put(Convert.getBytes(numAll));
        ...
        for (int i = 0; i < trainClasses.rows(); i++) {
            for (int k = 0; k < trainClasses.cols(); k++) {
            	...
            	BytePointer bytePointer = trainClasses.ptr(i, k);
                if (k == Convert.toInt(classes.ptr(i)))
                	bytePointer.put(Convert.getBytes(1f));
                else
                	bytePointer.put(Convert.getBytes(0f));
            }
        }
    ...
    public int saveTrainData() {
        ...
        String path = "D:/PlateDetect/train/chars2";
        ...
        path = "D:/PlateDetect/train/charsChinese";
